...
For the live stats such as battery percentage, online status, and my Bangalore map, I have set up a handful of scripts on a server that return this information via APIs. The battery status (homepage top-right) comes from Owntracks on my phone (which is sent to the server). I also use Owntracks to generate tracks and then map-match them using a self-hosted Valhalla instance, finally appending them to a PMTiles file which is served here using MapLibre. The online status is a simple 'heartbeat' script which runs periodically on my laptop pinging my server to notify that the laptop is active; if it doesn't receive a ping for a certain amount of time, it is assumed I am not working.
