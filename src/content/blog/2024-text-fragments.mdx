...
This post became longer than you probably have the patience to read, so here's a TL/DR index: 

1. [Background on Who Is My Neta](#some-background-on-who-is-my-neta); what we built and why we wanted to keep it alive.  
2. [Asking for permission](#you-shall-not-scrape) and not getting it.   
3. [Why it is difficult](#thinking-of-alternatives) to do this without scraping.  
4. Using [Chrome's Text Fragments](#enter-chrome-text-fragments) as a workaround to do more or less the same thing.  
5. [Final thoughts](#final-thoughts).  

## Some background on Who Is My Neta

In April, Opencity.in organized an online datajam around the theme of the 17th Lok Sabha elections, which I took part in along with my teammates Vivek and Pravar. Vivek had already been collating data on legislative activity of parliament members as well as data from MyNeta.info run by ADR on information such as criminal cases, education level, amount of declared assets, and so on (which they assemble through affidavits, which are also available as PDFs). Unfortunately, most are handwritten and cannot be OCR-ed. This was data on the **current** MPs on their way out. He had the novel idea of joining these two datasets and creating a single explorer for them. It sounded great to me. 

For context, this project almost feels like a rite of passage for people interested in open data, politics, and building visualizations. By no means had the ADR data not been visualized before; here's just some projects that have done similar things: 
...
All this to say, it had a lot of eyeballs on it, and we kept getting requests to update this with data for the candidates who would be contesting this year. Of course that would be useful, right? 

## You shall not scrape

To make sure that we were not doing something unwanted, we reached out to the open-data repository we had earlier scraped from and asked if this would be okay with them. We had made sure to appropriately credit the data to them to begin with, but having formal permission would have been nicer. 
...
Since the organized data by them was not out of reach, we began to think of alternate ways to keep our dashboard updated with the similar data for this year's candidates. The first thing to consider was assembling this data on our own. But have a look at how it can be filed: 
...
![](./images/images/2024-text-fragments/img-2024-text-fragments-202410-1.png)  
...
English, Hindi, handwritten, typed, skewed, rotated—there is no one way that the affidavits come in. Cleaning it is hard work, and to that end, I could possibly see why the folks who cleaned this want the data only on their platform (how that platform might hinder easy access for the casual user is another matter). 

OCR was out of the question. And there was no other format this data came in.

## Enter Chrome Text Fragments
...
`#:~:text=` will tell Chrome to look for the first piece of text that matches the parameter on the page. This was extremely useful! Now instead of just linking to the page, we could scroll to _and highlight_ the candidate you were interested in. Here's how it looks on the MyNeta website:
...
let link = `https://www.myneta.info/LokSabha2024/index.php?action=show_candidates&constituency_id=${constituencyID}${formatTextForHighlight(  
...
2. People use Chrome as their browser. [If the numbers are anything to go by](https://gs.statcounter.com/browser-market-share), this is a fairly okay assumption to make (yes, it is unfortunate).
...
**Update**: This has now made it into Firefox! Firefox allows you to open text fragment links but not create them (for now). 
...
When I wrote the URL-building logic and saw it work, I was incredibly excited. It's a stupid, stupid hack _but it works_. And making stuff like that is what makes all of this fun. Obviously I would much rather show things in one place and not have to do this, but oh well, `¯\_(ツ)_/¯` (Maybe the next group of programmers who go through this rite of passage next year will be the ones to do it.). 
