...
<SEO
  ...
  ogImage={`og?title=${encodeURIComponent(post.title)}&date=${encodeURIComponent(
    post.date
  )}`}
...
<article class="max-w-2xl mx-auto px-4 py-8">
  <header class="mb-8">
    ...
    <div class="flex items-center gap-2">
      ...
      <a href={post.url} target="_blank" rel="noopener noreferrer" class="text-primary hover:underline">
        {post.source}
      </a>
    </div>
  </header>
